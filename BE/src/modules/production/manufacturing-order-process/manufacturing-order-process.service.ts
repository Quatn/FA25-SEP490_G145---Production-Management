...
export class ManufacturingOrderProcessService {
  ...
  async updateOneProcess(
    ...
    // --- START: YÊU CẦU MỚI ---
    // RÀNG BUỘC: Nếu công đoạn trước bị HỦY, không cho phép cập nhật công đoạn này
    ...
    if (
      ...
      if (targetProcess.processNumber > 1) {
        ...
        // (Ràng buộc HỦY đã được kiểm tra ở trên)
        // Chỉ cần kiểm tra RUNNING hoặc COMPLETED
        if (
          !previousProcess ||
    ...
    if (newStatusFromDto) {
      if (newStatusFromDto === ProcessStatus.RUNNING) {
        ...
        if (newAmount > maxAmountForCompletion) {
          throw new BadRequestException(
            `Không thể hoàn thành khi số lượng (${newAmount}) vượt quá 110% cho phép (${maxAmountForCompletion.toFixed(
              0,
            )}).`,
          );
        }
