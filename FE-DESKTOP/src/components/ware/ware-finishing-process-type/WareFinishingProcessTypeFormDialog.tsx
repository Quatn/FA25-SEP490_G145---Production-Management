...
interface Props {
    ...
    onAdd: (data: WareFinishingProcessType) => void;
    onUpdate: (data: WareFinishingProcessType) => void;
}

type ErrorMap = Record<string, string>;

const WareFinishingProcessTypeFormDialog: React.FC<Props> = ({ isOpen, onClose, initialData, onAdd, onUpdate }) => {
    const emptyItem = { _id: "", code: "", name: "", description: "", note: "", createdAt: new Date(), updatedAt: new Date() } as WareFinishingProcessType;
    const [item, setItem] = useState<WareFinishingProcessType>(emptyItem);
        ...
        <Dialog.Root open={isOpen} onOpenChange={onClose} size={"lg"} scrollBehavior={"inside"}>
            <Portal>
                ...
                <Dialog.Positioner>
                    <Dialog.Content>
                        <Dialog.Header>
                            <Dialog.Title>{!initialData ? "Thêm Loại Hoàn Thiện Mã Hàng" : "Sửa Loại Hoàn Thiện Mã Hàng"}</Dialog.Title>
                        </Dialog.Header>
